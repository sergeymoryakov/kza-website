(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const a of o.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&s(a)}).observe(document,{childList:!0,subtree:!0});function i(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(n){if(n.ep)return;n.ep=!0;const o=i(n);fetch(n.href,o)}})();class u extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `}}customElements.define("link-icon-touch-white",v);class y extends HTMLElement{connectedCallback(){this.innerHTML=`
            <a class="link-home-white" href="#header">
                <span class="button-text">up to home</span>
            </a>
        `}}customElements.define("link-home-white",y);class b extends HTMLElement{static get observedAttributes(){return["title","icon","image","text"]}attributeChangedCallback(e,i,s){switch(e){case"title":this._title=s;break;case"icon":this._icon=s;break;case"image":this._image=s;break;case"text":this._text=JSON.parse(s);break}}connectedCallback(){this.classList.add("card-container"),this.innerHTML=`
                    <div
                        class="card"
        ...
        `;document.querySelector(".aircraft-3d").innerHTML=e,document.querySelector(".logo-3d").innerHTML=i,document.querySelector(".engine-3d").innerHTML=s}else{const e=`
                        <img
                        ...
                        />
        `;document.querySelector(".aircraft-3d").innerHTML=e}}window.addEventListener("load",I);function c(){g.classList.toggle(_),l.classList.toggle(r)}function p(){l.classList.toggle(r),T.classList.toggle(w),C.classList.toggle(E),d.classList.toggle(k),N.classList.toggle(x)}d.addEventListener("click",function(){p()});S.addEventListener("click",function(){p()});L.forEach(t=>{const e=document.createElement("card-container");e.setAttribute("title",t.title),e.setAttribute("icon",`icons/${t.icon}`),e.setAttribute("image",`images/${t.image}`),e.setAttribute("text",JSON.stringify(t.text)),M.appendChild(e)});
