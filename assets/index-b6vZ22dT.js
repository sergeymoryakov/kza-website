(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const a of o.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&i(a)}).observe(document,{childList:!0,subtree:!0});function s(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=s(n);fetch(n.href,o)}})();class p extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `}}customElements.define("link-icon-touch-white",f);class b extends HTMLElement{connectedCallback(){this.innerHTML=`
            <a class="link-home-white" href="#header">
                <span class="button-text">up to home</span>
            </a>
        `}}customElements.define("link-home-white",b);class y extends HTMLElement{static get observedAttributes(){return["title","icon","image","text"]}attributeChangedCallback(t,s,i){switch(t){case"title":this._title=i;break;case"icon":this._icon=i;break;case"image":this._image=i;break;case"text":this._text=JSON.parse(i);break}}connectedCallback(){this.classList.add("card-container"),this.innerHTML=`
                    <div
                        class="card"
        ...
        `;document.querySelector(".aircraft-3d").innerHTML=t,document.querySelector(".logo-3d").innerHTML=s,document.querySelector(".engine-3d").innerHTML=i}}window.addEventListener("load",I);function c(){g.classList.toggle(_),l.classList.toggle(r)}function u(){l.classList.toggle(r),T.classList.toggle(E),C.classList.toggle(k),d.classList.toggle(w),N.classList.toggle(x)}d.addEventListener("click",function(){u()});O.addEventListener("click",function(){u()});L.forEach(e=>{const t=document.createElement("card-container");t.setAttribute("title",e.title),t.setAttribute("icon",`icons/${e.icon}`),t.setAttribute("image",`images/${e.image}`),t.setAttribute("text",JSON.stringify(e.text)),M.appendChild(t)});
